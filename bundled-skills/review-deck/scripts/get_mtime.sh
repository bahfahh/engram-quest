...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" || "$OS" == "Windows_NT" ]]; then
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; ([DateTimeOffset](Get-Item '$FILE' -ErrorAction Stop).LastWriteTimeUtc).ToUnixTimeMilliseconds()"
