...
| `iframe` | Interact with local HTML | Dynamic simulations and timed systems | medium-hard |
...
Embed a local HTML challenge through an iframe. Use this only when the learner must manipulate a dynamic system or observe time-based behavior that native YAML mechanics cannot express.
...
Required fields:
- `type: iframe`
- `html`: vault-relative path to an HTML file generated by the skill

Optional fields:
- `height`: initial iframe height in pixels. The runtime clamps extreme values.

HTML contract:
- The file must be self-contained: inline CSS and JavaScript only.
- Do not use external `fetch`, CDN scripts, remote fonts, popups, `alert()`, or top-level navigation.
- When solved, call `window.parent.postMessage({ type: "engram-quest-solved", score: 0-100 }, "*")`.
- To resize, call `window.parent.postMessage({ type: "engram-quest-resize", height: 520 }, "*")`.
- Listen for `engram-quest-theme` messages and adapt colors when `event.data.dark` is true.
- The HTML file should live at `engram-quest/html/{quest-base}/{nodeId}.html`; keep `{nodeId}` stable when updating unchanged challenges.
